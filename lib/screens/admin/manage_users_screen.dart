...
class _ManageUsersScreenState extends State<ManageUsersScreen> {
  void _showAddUserDialog() {
    ...
    String newPassword = '';
    String newUsername = '';
    String newRole = 'user'; // default role to create
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setStateDialog) {
            ...
            return AlertDialog(
              title: const Text('Create New User/Teacher'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        // Only load standard users and teachers, admins should not manage teamIt
        stream: FirebaseFirestore.instance.collection('users').where('role', whereIn: ['user', 'teacher']).snapshots(),
        builder: (context, snapshot) {
          if (!snapshot.hasData) return const Center(child: CircularProgressIndicator());

          final docs = snapshot.data!.docs;
          if (docs.isEmpty) return const Center(child: Text('No standard users or teachers found.'));
